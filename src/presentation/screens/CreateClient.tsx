...
const CreateClient: React.FC = () => {
    ...
    const handleClick = async () => {
        ...
        // Verificar si se ingresaron caracteres no numéricos
        ...
        try {
            const facturaProps = {
                invoiceValue: parseFloat(sanitizedValue) // Usar sanitizedValue aquí
            };
            const nuevaFactura = new Factura(facturaProps);

            await facturaService.postFactura(nuevaFactura);
            setSnackbarMessage(`Factura emitida con valor: ${sanitizedValue}`); // Usar sanitizedValue aquí
            setOpenSnackbar(true);
        } catch (error) {
